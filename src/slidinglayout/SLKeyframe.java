...
import slidinglayout.SLConfig.Tile;

/**
 * A keyframe wraps a {@link SLConfig layout configuration} together with various parameters, such
 * as the ending and starting sides of new and old components, and the optional delays that can be
 * applied to the components to create some nice effects.
 *
...
public class SLKeyframe {
    ...
    private final Map<SLSide, List<Component>> cmpsWithStartSide =
            new EnumMap<SLSide, List<Component>>(SLSide.class);
    private final Map<SLSide, List<Component>> cmpsWithEndSide =
            new EnumMap<SLSide, List<Component>>(SLSide.class);
    ...
    /**
     * The duration parameter controls the duration of the transition of a component from its
     * current place to its target place in the keyframe layout configuration. It doesn't account
     * for any optional delay you may add to it.
     */
    ...
    /**
     * Sets the side of the screen from where the given new components will slide from. A new
     * component is a component that is present in this layout configuration but not on the previous
     * one.
     */
    ...
    /**
     * Sets the side of the screen from where every new component will slide from. A new component
     * is a component that is present in this layout configuration but not on the previous one.
     */
    ...
    /**
     * Sets the side of the screen from where the given old components will slide to. An old
     * component is a component that is not present in this layout configuration but was on the
     * previous one.
     */
    ...
    /**
     * Sets the side of the screen from where every old component will slide to. An old component is
     * a component that is not present in this layout configuration but was on the previous one.
     */
    ...
    /**
     * Sets the delay used by the given components before the transition from their current place to
     * their target one starts.
     */
    ...
    /**
     * Sets the delay used by the given components before the transition from their current place to
     * their target one starts. The delay is increased for each component in the list.
     */
    ...
    /**
     * Increases the delay used by the given components before the transition from their current
     * place to their target one starts.
     */
    ...
    /**
     * Increases the delay used by the given components before the transition from their current
     * place to their target one starts. The delay is increased for each component in the list.
     */
    ...
    /** Sets a callback that will be called at the end of the transition to this keyframe.s */
    ...
    private boolean isPartOf(Component c, Map<SLSide, List<Component>> map) {
        for (SLSide s : SLSide.values()) if (map.get(s).contains(c)) return true;
        return false;
    }
