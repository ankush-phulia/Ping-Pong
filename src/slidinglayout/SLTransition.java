...
/**
 * A transition consists of one or more keyframes (ie. configurations). The engine will move the
 * children of the panel from the current configuration to the first keyframe, and so on.
 *
 * <p>If a component is present on a keyframe but absent from the current configuration, it will be
 * considered to be a <b>new component</b>. If you don't specify anything, it will appear right at
 * its target place, which may not be very nice. Instead, you can set its starting side, the side
 * from where it should be brought into view to its target place. It will then slide from this side
 * to its target place.
 *
 * <p>Similarly, if a component is present in the current configuration but absent from the next
 * keyframe, it will be considered to be an <b>old component</b>. You can slide it out of the screen
 * gracefully by specifying its ending side.
 *
 * @see SLConfig
 * @see SLKeyframe
...
public class SLTransition {
    ...
    /**
     * Attempts to start the transition. Returns true if everything went right. If another
     * transition was already running, nothing happens and the method returns false;
     */
    public boolean play() {
